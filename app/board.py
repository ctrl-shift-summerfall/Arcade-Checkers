...
from app.variables import (VAR_BOARD_TILE_COLOR_BLACK, VAR_BOARD_TILE_COLOR_WHITE, VAR_CHECKER_COLOR_BLACK, VAR_CHECKER_COLOR_NOT_SET, VAR_CHECKER_COLOR_WHITE, VAR_CHECKER_TYPE_ORDINARY, 
                           VAR_SESSION_RENDER_MODE_OPENGL, VAR_SESSION_RENDER_MODE_TEXTURE)

# Internal scripts:
from app.scripts import (create_new_checker,)
            ...
            for column in self.grid[row]:
                if row in (0, 1, 2, 5, 6, 7):
                    ...
                    if row % 2 != 0 and column % 2 != 0 or row % 2 == 0 and column % 2 == 0:
                        ...
                        new_checker = create_new_checker(ch_color=spawn_color, ch_type=spawn_type, ch_grid_position=spawn_position)
